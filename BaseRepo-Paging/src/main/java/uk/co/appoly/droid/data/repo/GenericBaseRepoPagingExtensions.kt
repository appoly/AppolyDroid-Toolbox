 ...
 * [RootJsonPage] responses and converts them to [PageData] for easier
 * consumption by Paging components.
 *
 * Example usage:
 * ```kotlin
 * suspend fun fetchUserList(page: Int): APIResult<PageData<User>> =
 *     doPagedAPICall("fetchUserList") {
 ...
 * @param call Lambda that performs the API call and returns an [ApiResponse] with [RootJsonPage]
 * @return An [APIResult] wrapping [PageData] with the normalized pagination data
 */
inline fun <T : Any> GenericBaseRepo.doPagedAPICall(
	logDescription: String,
	call: () -> ApiResponse<RootJsonPage<T>>
